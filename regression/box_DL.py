...
def generate_rotated_rectangles(B=10, L=50, output_file="rectangles.txt", visualize=True):
    ...
    with open(output_file, "w") as f:
        ...
        while rect_id <= B:  # 用 while 确保有效矩形数量
            ...
            # gtx1 gty1 gtx2 gty2 gtx3 gty3 gtx4 gty4
            ...
            if visualize:
                ...
                for px, py in rotated_corners:
                    cv2.circle(img, (int(px), int(py)), 3, (0, 0, 255), 3)
...
# 示例
for idx in range(1):
    # output_file = f"rectangles_tmp.txt"
    # generate_rotated_rectangles(B=20, L=60, output_file=output_file, visualize=True)
    
    output_file = f"rectangles.txt"
    generate_rotated_rectangles(B=100000, L=60, output_file=output_file, visualize=False)
